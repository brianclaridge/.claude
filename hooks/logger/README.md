...
```json
{
  "log_base_path": "${CLAUDE_PATH}/.data/logs/claude_hooks"
}
```
...
Logs are written to:
```
${CLAUDE_PATH}/.data/logs/claude_hooks/{session_id}/{hook_event_name}/{timestamp}.json
```
...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "command": "uv run --directory ${CLAUDE_PATH}/hooks/logger python -m src"
      }
    ],
    "PostToolUse": [
      {
...
echo '{"hook_event_name": "PostToolUse", "session_id": "test123", "tool_name": "Bash"}' | \
  uv run --directory ${CLAUDE_PATH}/hooks/logger python -m src

# Verify log was created
cat ${CLAUDE_PATH}/.data/logs/claude_hooks/test123/PostToolUse/*.json
```
...
ls -la ${CLAUDE_PATH}/.data/logs/claude_hooks/

# Remove logs older than 7 days
find ${CLAUDE_PATH}/.data/logs/claude_hooks -type f -mtime +7 -delete

# Remove empty directories
find ${CLAUDE_PATH}/.data/logs/claude_hooks -type d -empty -delete
```
