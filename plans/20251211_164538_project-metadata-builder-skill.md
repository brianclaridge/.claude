...
Create a new skill `project-metadata-builder` that generates and maintains a comprehensive `~/.claude/projects.yml` registry. The skill will be invoked by `project-discovery-agent` and can run automatically, manually, or periodically.
...
- [ ] Create `~/.claude/config.yml` with default settings
...
description: Build and update project metadata registry in ~/.claude/projects.yml. Use when starting a session, after project analysis, or when user says "update project metadata", "refresh projects", "register project".
...
Dashboard application would read `~/.claude/projects.yml` directly.
