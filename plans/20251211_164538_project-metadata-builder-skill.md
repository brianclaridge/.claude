...
Create a new skill `project-metadata-builder` that generates and maintains a comprehensive `${PROJECTS_YML_PATH}` registry. The skill will be invoked by `project-discovery-agent` and can run automatically, manually, or periodically.

## New Files to Create

| File | Purpose |
|------|---------|
| `${CONFIG_YML_PATH}` | Global configuration for custom features |
| `${PROJECTS_YML_PATH}` | Project metadata registry (generated) |
...
- [ ] Create `${CONFIG_YML_PATH}` with default settings
...
description: Build and update project metadata registry in ${PROJECTS_YML_PATH}. Use when starting a session, after project analysis, or when user says "update project metadata", "refresh projects", "register project".
...
Dashboard application would read `${PROJECTS_YML_PATH}` directly.
